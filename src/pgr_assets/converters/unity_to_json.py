...
def object_to_json(obj):
    ...
    if isinstance(obj, classes.ComponentPair):
        ...
        return {
            "__type": type(obj).__name__,
            "__path_id": (
                None if obj.object_reader is None else obj.object_reader.path_id
            ),
        ...
        return {
            "m_Colors": None
            if obj.m_Colors is None
            else {k: object_to_json(v) for (k, v) in obj.m_Colors},
            "m_Floats": None
            if obj.m_Floats is None
            else {k: object_to_json(v) for (k, v) in obj.m_Floats},
            "m_Ints": None
            if obj.m_Ints is None
            else {k: object_to_json(v) for (k, v) in obj.m_Ints},
            "m_TexEnvs": None
            if obj.m_TexEnvs is None
            else {k: object_to_json(v) for (k, v) in obj.m_TexEnvs},
        }
...
def jsonify(obj: classes.Object):
    ...
    while queue:
        ...
        if isinstance(obj, classes.ComponentPair):
            queue.append(cast(classes.Object, obj.component))
