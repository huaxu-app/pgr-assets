...
def get_cue_references(sources: SourceSet, game_version: tuple[int, int]):
    ...
    sheet = extractors.get_text_asset(env, f'assets/temp/{source}/client/audio/cuesheet.tab.bytes', allow_binary_table_convert=True, game_version=game_version)
...
class CueRegistry:
    ...
    def init(self, sources: SourceSet):
        cue_sheets = {int(id): CueSheet(int(id), acb, awb) for id, acb, awb, _ in get_cue_references(sources, self.game_version)}
