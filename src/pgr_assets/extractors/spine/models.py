...
class SpineInfo:
    ...
    def valid(self):
        return (
            len(self.textures) > 0 and self.json is not None and self.atlas is not None
        )
    ...
    def write(self, output_dir: str):
        ...
        assert self.json is not None and self.atlas is not None
        name = self.name
...
class Spine:
    ...
    def to_json(self):
        ...
        animation_counts = Counter(
            x.default_animation for x in self.spines if x.default_animation is not None
        )
